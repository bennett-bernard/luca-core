...
from lumbago.models.money import Money
...
class Account(RecordModel):
    ...
    code: str = Field(
        ...
        description=(
            ...
            "cash.operating. Codes are unique within a Lumbago data store."
        ),
        examples=["1000"],
    )
    name: str = Field(
...
class Journal(RecordModel):
    ...
    code: str = Field(
        ...
        pattern=r"^[A-Za-z0-9][A-Za-z0-9._-]*$",
        description=(
            "Short human-facing identifier for the journal, unique within a Lumbago data "
            "store."
        ),
        examples=["GENERAL"],
    )
...
class JournalLine(LumbagoModel):
...
class BaseTransaction(RecordModel):
    ...
    transaction_date: date = Field(
        description=(
            ...
            "the timestamp when it was entered into Lumbago."
        ),
        examples=["2026-08-22"],
    )
